...
- Only ONE serial_text device allowed. Fixed pins per chip: C6/C3=RX4/TX5, S3 SuperMini=RX GPIO2/TX GPIO1.
- JAFR servo bridge default: use/register `c3servo` as `serial_text` at 9600 baud.
- Wire brain S3 GPIO1 TX to ESP32D GPIO16 RX2, brain S3 GPIO2 RX to ESP32D GPIO17 TX2, and share GND.
- `serial_send` only proves the command was written to UART. Confirm movement or read `c3servo` for the ESP32D reply.
...
- `serial_send` - Send text over UART: `{"tool":"serial_send","text":"PING"}`
