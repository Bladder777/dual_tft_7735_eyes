...
| ESP32-S3 SuperMini | GPIO 2 | GPIO 1 |
| ESP32-C3 | GPIO 4 | GPIO 5 |

Pins are fixed per chip - no configuration needed. Connect your serial device's TX to the RX pin and vice versa. Don't forget a common ground. For the JAFR ESP32D servo bridge, wire brain S3 GPIO1 TX to ESP32D GPIO16 RX2 and brain S3 GPIO2 RX to ESP32D GPIO17 TX2.

#### Registration

```
device_register(name="c3servo", type="serial_text", baud=9600)
```
...
#### Example: ESP32D Servo Bridge

Wire the ESP32D's TX2 to the brain S3's UART1 RX pin, cross TX/RX, and share ground:

```
You:  "Connect the ESP32D servo bridge on serial at 9600 baud"

AI:   device_register(name="c3servo", type="serial_text", baud=9600)
      -> Registered serial_text sensor 'c3servo' at 9600 baud (RX=2 TX=1 on ESP32-S3 SuperMini)

You:  "Every 30 seconds, send STATUS to the ESP32D servo controller.
       Alert me on Telegram when it reports a change."

AI:   rule_create(rule_name="poll", sensor_name="chip_temp",
                  condition="always", interval_seconds=30,
                  on_action="serial_send", on_serial_text="STATUS")
      -> Rule created: rule_01 'poll'

      rule_create(rule_name="alert", sensor_name="c3servo",
                  condition="change",
                  on_action="telegram",
                  on_telegram_message="ESP32D: {c3servo:msg}")
      -> Rule created: rule_02 'alert'
```

The first rule sends `STATUS\n` to the ESP32D every 30 seconds. The ESP32D replies with a line of text. The second rule fires when that message changes and sends the text to Telegram.
...
Via LLM tool:
```
sensor_read(name="c3servo")  ->  c3servo: 0.0  (last: 'PONG')
```

Via serial:
```
/devices
  c3servo [serial_text] 9600baud  = 0.0   msg='PONG'
```
