...
#if !defined(CONFIG_IDF_TARGET_ESP32) && __has_include("driver/temperature_sensor.h")
#include "driver/temperature_sensor.h"
#define WIRECLAW_HAS_TEMP_SENSOR 1
#else
#define WIRECLAW_HAS_TEMP_SENSOR 0
#endif
...
#define EYE_SYNC_INTERVAL_MS 16
#define EYE_SYNC_MAGIC       0xE773
#define EYE_SYNC_VERSION     1
#define EYE_SCLERA_WIDTH     200
#define EYE_SCLERA_HEIGHT    200
#define EYE_SCREEN_WIDTH     128
#define EYE_SCREEN_HEIGHT    128
#define EYE_IRIS_MIN         90
#define EYE_IRIS_MAX         130
...
static const uint8_t EYE_SYNC_BROADCAST_ADDR[] =
    {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
static bool eyeSyncReady = false;
static uint32_t eyeSyncSeq = 0;
static uint32_t eyeSyncLastMs = 0;
static uint32_t eyeBlinkStartMs = 0;
static uint32_t eyeNextBlinkMs = 2500;
static bool eyeBlinking = false;
static bool eyeMoveInMotion = false;
static int16_t eyeOldX = 512;
static int16_t eyeOldY = 512;
static int16_t eyeNewX = 512;
static int16_t eyeNewY = 512;
static uint32_t eyeMoveStartUs = 0;
static uint32_t eyeMoveDurationUs = 0;
...
void led(uint8_t r, uint8_t g, uint8_t b) {
    ...
    digitalWrite(RGB_BUILTIN, (r || g || b) ? HIGH : LOW);
#endif
...
static int16_t mapEyeCoord(int16_t raw, int16_t maxPixel) {
    return (int16_t)(((int32_t)raw * maxPixel) / 1023);
}

static uint8_t easeInOut8(uint32_t elapsed, uint32_t duration) {
    if (duration == 0 || elapsed >= duration) return 255;
    uint32_t x = (elapsed * 255U) / duration;
    return (uint8_t)((3U * x * x - (2U * x * x * x) / 255U) / 255U);
}

static void chooseNewEyeTarget() {
    int16_t dx;
    int16_t dy;
    uint32_t d;
    ...
    do {
        ...
        dx = (eyeNewX * 2) - 1023;
        dy = (eyeNewY * 2) - 1023;
        d = (uint32_t)(dx * dx + dy * dy);
    } while (d > (1023UL * 1023UL));
}
...
static void updateEyeMotion(int16_t *scleraX, int16_t *scleraY) {
    ...
    int16_t rawY = eyeOldY;

    if (eyeMoveInMotion) {
        if (dt >= eyeMoveDurationUs) {
            eyeMoveInMotion = false;
            eyeMoveStartUs = nowUs;
            eyeMoveDurationUs = random(120000, 900000);
            eyeOldX = eyeNewX;
            eyeOldY = eyeNewY;
            rawX = eyeOldX;
            ...
            uint8_t e = easeInOut8(dt, eyeMoveDurationUs);
            rawX = eyeOldX + (((eyeNewX - eyeOldX) * (int16_t)e) / 255);
            rawY = eyeOldY + (((eyeNewY - eyeOldY) * (int16_t)e) / 255);
        }
    } else if (dt > eyeMoveDurationUs) {
        chooseNewEyeTarget();
        eyeMoveDurationUs = random(72000, 144000);
        eyeMoveStartUs = nowUs;
        eyeMoveInMotion = true;
    }
...
static uint16_t updateEyeIris(uint32_t now) {
    if (now >= eyeNextIrisMs) {
        eyeIrisTarget = random(EYE_IRIS_MIN, EYE_IRIS_MAX + 1);
        eyeNextIrisMs = now + random(350, 1200);
    }

    eyeIrisScale = (uint16_t)(((uint32_t)eyeIrisScale * 7U + eyeIrisTarget) / 8U);
    return eyeIrisScale;
}

static void eyeSyncInit() {
    if (eyeSyncReady) {
        esp_now_deinit();
        eyeSyncReady = false;
    }
    ...
    if (esp_now_init() != ESP_OK) {
        Serial.printf("ESP-NOW eye sync init failed\n");
        return;
    }

    esp_now_peer_info_t peer = {};
    memcpy(peer.peer_addr, EYE_SYNC_BROADCAST_ADDR, sizeof(EYE_SYNC_BROADCAST_ADDR));
    peer.channel = 0; /* use current WiFi STA channel */
    peer.encrypt = false;

    esp_err_t addErr = esp_now_add_peer(&peer);
    if (addErr != ESP_OK && addErr != ESP_ERR_ESPNOW_EXIST) {
        Serial.printf("ESP-NOW eye sync broadcast peer failed: %d\n", addErr);
        esp_now_deinit();
        return;
    }
    ...
    eyeSyncSeq = 0;
    eyeSyncLastMs = 0;
    eyeNextBlinkMs = millis() + 2500;
    eyeMoveStartUs = micros();
    eyeMoveDurationUs = 1;
    eyeNextIrisMs = millis() + 500;
    Serial.printf("ESP-NOW eye sync: broadcasting on WiFi channel %d\n", WiFi.channel());
}

static void eyeSyncTick() {
    if (!eyeSyncReady || WiFi.status() != WL_CONNECTED) return;

    uint32_t now = millis();
    if (now - eyeSyncLastMs < EYE_SYNC_INTERVAL_MS) return;
    eyeSyncLastMs = now;

    if (!eyeBlinking && now >= eyeNextBlinkMs) {
        eyeBlinking = true;
        eyeBlinkStartMs = now;
    }

    uint8_t upper = 118;
    uint8_t lower = 254 - upper;
    if (eyeBlinking) {
        uint32_t elapsed = now - eyeBlinkStartMs;
        if (elapsed >= 180) {
            eyeBlinking = false;
            eyeNextBlinkMs = now + 2500 + (uint32_t)((eyeSyncSeq * 379) % 3500);
        } else {
            uint32_t s = elapsed < 90 ? elapsed : (180 - elapsed);
            uint8_t lid = (uint8_t)(118 + ((uint32_t)136 * s) / 90);
            upper = lid;
            lower = lid;
        }
    }
    ...
    int16_t scleraX;
    int16_t scleraY;
    updateEyeMotion(&scleraX, &scleraY);

    packet.magic = EYE_SYNC_MAGIC;
    packet.version = EYE_SYNC_VERSION;
    packet.side = 0;
    packet.seq = ++eyeSyncSeq;
    packet.scleraX = scleraX;
    packet.scleraY = scleraY;
    packet.irisScale = updateEyeIris(now);
    packet.upperThreshold = upper;
    packet.lowerThreshold = lower;

    esp_now_send(EYE_SYNC_BROADCAST_ADDR, (const uint8_t *)&packet, sizeof(packet));
}
