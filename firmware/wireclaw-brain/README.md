...
```
You (on Telegram):
    "Set up the ESP32D servo bridge at 9600 baud, call it c3servo"

Serial:  device_register({"name":"c3servo","type":"serial_text","baud":9600})
         SerialText: UART1 at 9600 baud (RX=2 TX=1 on ESP32-S3 SuperMini)
         -> Registered serial_text sensor 'c3servo' at 9600 baud (RX=2 TX=1)
```

Now WireClaw can send commands and receive data over the UART:

```
You:  "Ping the ESP32D servo controller"

Serial:  serial_send({"text":"PING"})
         -> Sent to serial: PING

         [SerialText] 'PONG' -> val=0.0 msg='PONG'

         sensor_read({"name":"c3servo"})
         -> c3servo: 0.0  (last: 'PONG')

WireClaw: "The ESP32D servo controller replied PONG."
```

And rules work on serial data just like any other sensor:

```
You:  "Alert me on Telegram when the ESP32D servo status changes."

Serial:  rule_create(sensor_name="c3servo", condition="change",
                     on_action="telegram",
                     on_telegram_message="ESP32D: {c3servo:msg}")

         -> Rule created: rule_01 'servo status' - c3servo changes
```
